...
beforeAll(async () => {
  ...
  const user1 = await User.create({ name: 'Bertrand', email: 'bertrand@mail.com', password: passwordHash });
  const user2 = await User.create({ name: 'Jean', email: 'jean@mail.com', password: passwordHash });
...
describe('Transfer endpoint', () => {
  ...
  test('should fail if amount is not a number', async () => {
    const res = await request(app)
      .post(`/accounts/${fromAccountId}/transfer`)
      .set('Authorization', `Bearer ${token}`)
      .send({ toAccountId, amount: 'notANumber' });
    expect(res.status).toBe(400);
